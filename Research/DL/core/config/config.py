...
class CfgNode(_CfgNode):
    """
    class:`yacs.config.CfgNode` 的扩展版本
    包含以下的扩展特性：
    1. `merge_from_file`方法支持"_BASE_" key，能够从多个基础配置文件中继承属性。
    2. 以“COMPUTED_”开头的key被视为仅插入的“computed”属性。无论CfgNode是否被冻结，都可以插入它们。
    3. 当allow_unsafe=True时，将支持在配置中计算表达式的pyyaml标签。
